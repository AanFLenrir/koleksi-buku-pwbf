...
@section('title', 'Point of Sales - FarmNex')
...
@push('styles')
    ...
    <style>
        /* ==========================================================
           FARM NEX EPIC – Point of Sales (Natural + Futuristik)
           ========================================================== */
        ...
        .cloud-farm-pos::before,
        .cloud-farm-pos::after {
            content: '' !important;
        ...
        .cloud-farm-pos-1::before {
            width: 70px !important;
            height: 70px !important;
            top: -30px !important;
            left: 20px !important;
        }
        .cloud-farm-pos-1::after {
        ...
        .cloud-farm-pos-2::before {
            width: 90px !important;
            height: 90px !important;
            top: -40px !important;
            left: 30px !important;
        }
        .cloud-farm-pos-2::after {
        ...
        .walking-animals-pos i {
            ...
            animation: bounceAnimalPos 1.2s ease-in-out infinite alternate !important;
        }
        ...
        #farm-pos-page .form-select:focus {
            border-color: #ffd700 !important;
            box-shadow: 0 0 25px rgba(255, 215, 0, 0.15) !important;
            background: rgba(255, 255, 255, 0.8) !important;
        }

        #farm-pos-page .form-control:disabled,
        #farm-pos-page .form-control[readonly] {
        ...
        #farm-pos-page .form-check-label small {
        ...
        #farm-pos-page .btn-danger:hover {
            ...
            box-shadow: 0 6px 20px rgba(255, 82, 82, 0.3) !important;
        }
        ...
        #farm-pos-page .table .form-control-sm {
            background: rgba(255, 255, 255, 0.6) !important;
            border: 1px solid rgba(255, 255, 255, 0.3) !important;
            border-radius: 50px !important;
            color: #1b3a2b !important;
        ...
        #farm-pos-page .table .form-control-sm:focus {
            border-color: #ffd700 !important;
        ...
        @media (max-width: 768px) {
            ...
            .fence-farm-pos { display: none !important; }
            .grass-farm-pos { height: 30px !important; }
            .cloud-farm-pos { display: none !important; }
            .sun-farm-pos { width: 40px !important; height: 40px !important; top: 15px !important; right: 20px !important; }
        }
    </style>
@endpush
...
@section('content')
    ...
    <div id="farm-pos-page">
        ...
        {{-- Elemen Dekoratif --}}
        <div class="sun-farm-pos"></div>
        <div class="cloud-farm-pos cloud-farm-pos-1"></div>
        <div class="cloud-farm-pos cloud-farm-pos-2"></div>
        <div class="grass-farm-pos"></div>
        <div class="fence-farm-pos">
                    ...
                    <div class="card shadow-sm border-0 rounded-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" id="inputKode" class="form-control" placeholder="Scan / ketik kode..." autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-semibold">
                                    <i class="fas fa-money-bill-wave"></i> Harga Barang
                                </label>
                                <input type="text" id="inputHarga" class="form-control" readonly>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">
                                    <i class="fas fa-sort-amount-up"></i> Jumlah
                                </label>
                ...
                <div class="col-lg-8">
                    <div class="card shadow-sm border-0 rounded-4">
                        <div class="card-header bg-white border-bottom fw-semibold text-primary d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('pos.riwayat') }}" class="btn btn-outline-primary btn-sm">
                                <i class="fas fa-history me-1"></i>Riwayat Transaksi
                            </a>
                        </div>
                        <div class="card-body">
                            <table class="table table-hover align-middle" id="tabelPOS">
                                <thead>
                                    <tr>
    ...
    <script>
        ...
        document.getElementById('inputKode').addEventListener('keydown', function(e) {
            ...
            axios.post('{{ route("pos.cari") }}', { kode: kode, _token: '{{ csrf_token() }}' })
                ...
                .catch(function() {
                    Swal.fire({ icon: 'error', title: 'Tidak Ditemukan', text: 'Kode barang tidak ada di database.', background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                });
        });
                ...
                axios.post('{{ route("pos.bayar") }}', { items: items, payment_method: 'tunai', _token: '{{ csrf_token() }}' })
                    .then(function(res) {
                        Swal.fire({
                            icon: 'success',
                            title: '✅ Berhasil!',
                            html: `Transaksi tunai berhasil!<br><strong>Order:</strong> ${res.data.order_code}<br><strong>Customer:</strong> ${res.data.customer}<br><strong>Total:</strong> Rp ${parseInt(res.data.total).toLocaleString('id-ID')}`,
                    ...
                    .catch(function() {
                        Swal.fire({ icon: 'error', title: 'Error!', text: 'Transaksi gagal disimpan.', background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                    ...
                    snap.pay(snapToken, {
                        onSuccess: function(result) {
                            ...
                            Swal.fire({
                                ...
                                html: `<strong>Order:</strong> ${orderCode}<br><strong>Customer:</strong> ${customer}<br><strong>Total:</strong> Rp ${parseInt(totalVal).toLocaleString('id-ID')}<br><strong>Metode:</strong> ${result.payment_type}<br><br><p class="mb-1 fw-semibold">QR Code Pesanan:</p><img src="${qrUrl}" alt="QR Code ${orderCode}" style="width:180px;height:180px;border:1px solid #dee2e6;border-radius:8px;padding:8px;">`,
                                background: 'rgba(255,255,255,0.9)',
                                backdrop: 'blur(10px)',
                        ...
                        onPending: function() {
                            Swal.fire({ icon: 'info', title: '⏳ Menunggu Pembayaran', html: `Order <strong>${orderCode}</strong> sedang menunggu pembayaran.<br>Silakan selesaikan pembayaran Anda.`, background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                            resetKeranjang();
                        },
                        onError: function() {
                            Swal.fire({ icon: 'error', title: '❌ Gagal!', text: 'Pembayaran gagal. Silakan coba lagi.', background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                            btn.disabled = false; btn.innerHTML = '<i class="fas fa-money-bill-wave me-2"></i>Bayar';
                        },
                        onClose: function() {
                            Swal.fire({ icon: 'warning', title: 'Popup Ditutup', text: 'Kamu menutup popup sebelum pembayaran selesai.', confirmButtonText: 'OK', background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                            btn.disabled = false; btn.innerHTML = '<i class="fas fa-money-bill-wave me-2"></i>Bayar';
                        }
                    });
                })
                .catch(function() {
                    Swal.fire({ icon: 'error', title: 'Error!', text: 'Gagal menghubungi server pembayaran.', background: 'rgba(255,255,255,0.9)', backdrop: 'blur(10px)' });
                    btn.disabled = false; btn.innerHTML = '<i class="fas fa-money-bill-wave me-2"></i>Bayar';
                });
        }
    </script>
